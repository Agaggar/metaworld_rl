#!/usr/bin/env python3
"""
Create a 1x1 matplotlib heatmap grid for PPO ablations from W&B runs.
Only one column (LunarLanderContinuous-v3) and one row (PPO).
...
ENV_COLS = [
    ("LunarLanderContinuous-v3", "LunarLanderContinuous"),
]

column_titles = ["LunarLanderContinuous: Target=200"]
...
def main() -> None:
    ...
    p.add_argument("--project", type=str, default="lunar_sweep")
    p.add_argument("--metric", type=str, default="eval/episode_return_mean")
    p.add_argument("--out", type=str, default=str("results/heatmap_lunarlander_sweep.png"))
    p.add_argument(
        "--max-runs",
        type=int,
        default=100,
        help="Safety cap for scan size (only runs in the expected PPO ablation grid are included).",
    )
    args = p.parse_args()
    ...
    for row, algorithm in tqdm(enumerate(ALG_ROWS), desc="Rows"):
        for col, (env_id, env_short) in tqdm(enumerate(ENV_COLS), desc="Columns"):
            if len(ALG_ROWS) == 1 and len(ENV_COLS) == 1:
                ax = axes
            else:
                ax = axes[row, col]
    ...
    fig.suptitle("Final reward heatmaps for LunarLander", fontsize=20, y=0.985)
